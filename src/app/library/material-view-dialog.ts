...
@Component({
  ...
  template: `
    <div class="flex justify-between items-start px-4 sm:px-6 py-4 border-b w-full box-border relative">
      <h2 mat-dialog-title class="m-0 text-lg sm:text-xl font-bold text-gray-800 wrap-break-word line-clamp-2 flex-1 pr-10 leading-tight block">{{ data.title }}</h2>
    ...
    <mat-dialog-content class="mat-typography p-4 sm:p-6 custom-scrollbar bg-gray-50/50 min-h-[50vh] overflow-x-hidden flex flex-col box-border">
      <article class="print-container prose prose-sm sm:prose-base prose-slate prose-headings:text-primary prose-a:text-accent w-full max-w-none bg-white p-4 sm:p-8 md:p-10 rounded-xl shadow-sm border border-gray-100 box-border wrap-break-word" [innerHTML]="parsedContent">
      </article>
    </mat-dialog-content>
